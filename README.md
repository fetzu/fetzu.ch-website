...
Blog posts live at `/blog/YYYYMMDD_slug/`, straight from the file name. Each post also answers on `/blog/YYYY/MM/DD/slug/` and `/blog/slug/`, courtesy of a [`_redirects` file](layouts/home.redirects) Hugo generates for Cloudflare at build time; extra names can be declared per post with `aliases` in the front matter (see [the archetype](archetypes/post.md)). `/blog/YYYY/`, `/blog/YYYY/MM/` and `/blog/YYYY/MM/DD/` list the posts of that year, month or day, built by a [content adapter](content-extra/blog/_content.gotmpl) that lives in this repo and gets union-mounted into the content submodule (so the content repo stays content-only). Slugs have to stay unique for the dateless shortcut to make sense; on a clash, the older post wins.
